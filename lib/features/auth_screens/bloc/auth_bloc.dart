...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  AuthBloc() : super(AuthState()) {
    on<LoginEvent>((LoginEvent event, emit) async {
      ...
      final result = await AuthRequests().login(event.userName, event.password);
    ...
    on<RegisterEvent>((event, emit) async {
      emit(state.copyWith(status: AuthStatus.loading));
      final result = await AuthRequests()
          .register(username: event.username, password: event.password, email: event.email, phone: event.phone);
      result.fold((l) => emit(state.copyWith(status: AuthStatus.failed)), (r) {
        emit(state.copyWith(status: AuthStatus.success));
        GlobalFunctions().setUserInfo(user: r.user!, accessToken: r.jwt);
      });
